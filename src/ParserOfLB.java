...
/**
 * Created by zhanglizhong on 3/23/17.
 */
public class ParserOfLB {
    private  HashMap<String, ArrayList<Double>> data;
    private ArrayList<Integer> class_name;

    public ParserOfLB(File hamFolder, File spamFolder) throws Exception{
        String []hamFiles = hamFolder.list();
        String []spamFiles = spamFolder.list();
        int count = 0;

        this.class_name = new ArrayList<>(Collections.nCopies(hamFiles.length + spamFiles.length, 0));
            ...
            while(eachLine != null){
                ...
                for (String t : eachLine.replaceAll("^a-zA-Z", "").replaceAll("\\p{Punct}+", "").toLowerCase().split(" ")) {
                    if(data.containsKey(t)){
                        ...
                        ArrayList <Double>nums_words = new ArrayList<>(Collections.nCopies(hamFiles.length + spamFiles.length, 0.0));
    ...
    public ParserOfLB(File hamFolder, File spamFolder, File stopword) throws Exception{
        String []hamFiles = hamFolder.list();
        String []spamFiles = spamFolder.list();
        int count = 0;
